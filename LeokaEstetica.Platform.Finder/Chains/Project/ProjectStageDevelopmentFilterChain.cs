...
/// <summary>
/// Класс фильтра по стадии проекта (разработка).
/// </summary>
public class ProjectStageDevelopmentFilterChain : BaseProjectsFilterChain
{
    /// <summary>
    /// Метод фмильтрует проекты по стадии "разработка".
    /// </summary>
    ...
    {
        // Если фильтр не по стадии проекта "разработка", то передаем следующему по цепочке.
        if (!filters.ProjectStages.Contains(FilterProjectStageTypeEnum.Development))
        {
        ...
        var query = new TermQuery(new Term(ProjectFinderConst.PROJECT_STAGE_SYSNAME,
            FilterProjectStageTypeEnum.Development.ToString()));
