...
/// <summary>
/// Метод фмильтрует проекты по стадии "запуск".
/// </summary>
/// <param name="filters">Фильтры.</param>
/// <param name="projects">Список проектов.</param>
/// <returns>Список проектов после фильтрации.</returns>
public class ProjectStageStartFilterChain : BaseProjectsFilterChain
{
    /// <summary>
    /// Метод фмильтрует проекты по стадии "запуск".
    /// </summary>
    ...
    {
        // Если фильтр не по стадии проекта "запуск", то передаем следующему по цепочке.
        if (!filters.ProjectStages.Contains(FilterProjectStageTypeEnum.Start))
        {
        ...
        var query = new TermQuery(new Term(ProjectFinderConst.PROJECT_STAGE_SYSNAME,
            FilterProjectStageTypeEnum.Start.ToString()));
