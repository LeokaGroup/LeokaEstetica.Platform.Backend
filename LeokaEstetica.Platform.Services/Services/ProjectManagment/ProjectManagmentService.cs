		...
		try
		{
            ...
            */
			...
			if (new[] { SearchAgileObjectTypeEnum.Task, SearchAgileObjectTypeEnum.Error }.Contains(taskType))
			{
				...
				if (addedProjectTask.TaskStatusId == (int)SearchAgileObjectTypeEnum.Task)
				{
					await _hubNotificationService.Value.SendNotificationAsync("Все хорошо",
						...
						NotificationLevelConsts.NOTIFICATION_LEVEL_SUCCESS, "SendNotifySuccessCreateProjectTask",
						userCode, UserConnectionModuleEnum.ProjectManagement);
				}
			...
			if (taskType == SearchAgileObjectTypeEnum.Epic)
			{
				...
				await _hubNotificationService.Value.SendNotificationAsync("Все хорошо",
						$"Эпик успешно создан.",
						NotificationLevelConsts.NOTIFICATION_LEVEL_SUCCESS, "SendNotifySuccessCreateProjectEpic",
						userCode, UserConnectionModuleEnum.ProjectManagement);
			...
			if (taskType == SearchAgileObjectTypeEnum.Story)
			{
				...
				// Уведомление об успешном создании истории
				await _hubNotificationService.Value.SendNotificationAsync("Все хорошо",
						$"История успешно создан.",
						NotificationLevelConsts.NOTIFICATION_LEVEL_SUCCESS, "SendNotifySuccessCreateProjectStory",
						userCode, UserConnectionModuleEnum.ProjectManagement);
